 ...
 * 
 * 機能:
 * - 既存レシート編集フォーム（ReceiptInputPageと同じフロー）
 * - レシート更新ロジック
 * - タイムスタンプの更新
 */
function ReceiptEditPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!tripId || !receiptId) {
        setError('旅行IDまたはレシートIDが指定されていません')
        setIsLoading(false)
        return
      }
      ...
      try {
        ...
        // レシートを検索
        const foundReceipt = data.receipts.find(r => r.id === receiptId)
        if (!foundReceipt) {
        ...
        setTitle(foundReceipt.title || '')
        setPayerId(foundReceipt.payerId)
        setTargetMode(foundReceipt.targetMode)
        setSelectedMemberIds(foundReceipt.selectedMemberIds)
        setSplitMode(foundReceipt.splitMode)

        // 比率入力の設定
        if (foundReceipt.ratioInputs) {
          setRatioInputs(foundReceipt.ratioInputs)
        } else {
          setRatioInputs(data.members.map(m => ({ memberId: m.id, ratio: 100 })))
        }

        // 金額指定入力の設定
        if (foundReceipt.fixedInputs) {
          setFixedInputs(foundReceipt.fixedInputs)
        } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // レシート更新（要件3.4: updatedAtタイムスタンプを更新）
      ...
      const updatedTrip: Trip = {
        ...trip,
        receipts: trip.receipts.map(r => r.id === receipt.id ? updatedReceipt : r),
        updatedAt: new Date().toISOString()
      }
      ...
      // レシート一覧ページへ戻る
      navigate(`/trip/${tripId}/receipts`)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'レシートの更新に失敗しました')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const updatedTrip: Trip = {
        ...trip,
        receipts: trip.receipts.filter(r => r.id !== receipt.id),
        updatedAt: new Date().toISOString()
      }
      ...
      setError(err instanceof Error ? err.message : 'レシートの削除に失敗しました')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const handleSubgroupSelect = (subgroupId: string | null) => {
    ...
    if (subgroupId && trip) {
      ...
      if (subgroup) {
        ...
        // 比率配分モードの場合、サブグループのデフォルト配布比率で比率入力を事前入力
        if (splitMode === 'ratio') {
  ...
  const handleSplitModeChange = (mode: 'equal' | 'ratio' | 'fixed') => {
    ...
    if (mode === 'ratio' && trip) {
      ...
      // サブグループが選択されている場合、サブグループのデフォルト配布比率を使用
      ...
      // サブグループが選択されていない場合、メンバーのデフォルト配布比率を使用
  ...
  if (!trip || !receipt) {
    return (
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
        <p style={{ color: '#c00' }}>{error || 'データが見つかりません'}</p>
        <button
          onClick={() => navigate(`/trip/${tripId}/receipts`)}
          style={{
            padding: '10px 20px',
            fontSize: '16px',
